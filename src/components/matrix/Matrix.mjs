...
var cell = Tailwind$RescriptMonorepo.twStyle([
      ...
      Tailwind$RescriptMonorepo.border("v1"),
...
var row = Tailwind$RescriptMonorepo.twStyle([
      Tailwind$RescriptMonorepo.flex,
      Tailwind$RescriptMonorepo.justifyCenter
    ]);
...
function isOffsuit(string) {
  return string.endsWith("o");
}

function isSuited(string) {
  return string.endsWith("s");
}

function Matrix(Props) {
  var renderRow = function (array) {
    return Belt_Array.map(array, (function (combination) {
                  return React.createElement("div", {
                              className: Tailwind$RescriptMonorepo.merge([cell])
                            }, Utils$RescriptMonorepo.str(combination));
                }));
  };
  var renderMatrix = Belt_Array.map(rangeMatrix, (function (matrixRow) {
          return React.createElement("div", {
                      className: CssJs.merge([row])
                    }, renderRow(matrixRow));
        }));
  return React.createElement("div", {
              className: CssJs.merge([matrix])
            }, renderMatrix);
}
